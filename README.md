...
## 🆕 最新アップデート (ver 1.0.10) (2025年8月13日)

### ✨ マルチピン選択と一括編集システム
- **チェックボックス選択**: 直感的なチェックボックスベースの複数ピン選択
- **範囲選択**: Shift+クリックによる連続ピンの範囲選択
- **一括シグナル編集**: 選択された複数ピンへの信号名一括割り当て
- **分離されたUI**: 選択操作と3Dナビゲーションの明確な分離

### 🔧 XDC制約エクスポートの改善 (Issue #37, #38)
- **IOSTANDARD優先順位**: ユーザー選択値を優先するスマートなエクスポート
- **Drive/Slew制約**: Output/InOutピンのみに正しく適用
- **デフォルト値表示**: "---DriveStrength---", "---SlewRate---"形式の分かりやすい未設定表示

### 🛠️ 開発者体験の向上
- **統一デバッグシステム**: カテゴリ別ログ出力とパフォーマンス監視
- **型安全性**: TypeScript strict mode対応
- **保守性**: 組織化されたコードベースと一元化されたデバッグ機能

### 🎯 過去のアップデート (ver 1.0.7) (2025年8月12日)

### ✨ I/O設定GUIの新機能 (Issue #34)
- **デュアルドロップダウンレイアウト**: 直感的な電圧とI/O規格の選択
- **リアルタイム検証**: 電圧とI/O規格間の互換性を自動チェック
- **折りたたみ可能な詳細設定**: 詳細なピン設定用の展開可能なパネル
- **視覚的フィードバック強化**: プレースホルダーと視覚的インジケータの改善

### 📊 I/O規格の拡張サポート
- **高速規格**: LVDS_18, LVDS_25, LVDS_33, LVDS_E_25, LVPECL_25, LVPECL_33

### 🔧 リストモード改善
- **列ヘッダーソート**: 列ヘッダーをクリックしてソート、視覚的インジケータ付き (▲▼↕)
- **一括編集機能の強化**: Direction、Voltage、I/O Standardの包括的な一括割り当て
- **高度なBANKフィルタリング**: Show All/Hide All機能付きのインタラクティブなトグルボタン
- **ワークフロー改善**: Clear Selectionボタンと状態管理の向上
...
### メインインターフェース
FPGAピンアサインメントの直感的なGUIインターフェースです。

![メインGUI](docs/picture/GUI1.png)

### ピン詳細編集
ピン単位での詳細設定とリアルタイム検証機能です。
